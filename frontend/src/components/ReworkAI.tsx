...
export default function ReworkAI() {
  ...
  const HannahAvatar = ({ size = 'md', className = '' }: { size?: 'sm' | 'md' | 'lg', className?: string }) => {
    const sizeClasses = {
      sm: 'w-6 h-6 text-xs',
      md: 'w-8 h-8 text-sm',
    ...
    const avatarUrl = "https://api.dicebear.com/7.x/avataaars/svg?seed=Hannah&backgroundColor=b6e3f4&accessories=prescription02&top=longHairStraight&hairColor=brunette&clothingColor=3c4f5c&clothing=blazerShirt";

    return (
      <div className={`${sizeClasses[size]} rounded-full overflow-hidden bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center ${className}`}>
        {!imageError ? (
          <img
            src={avatarUrl}
            alt="Hannah Support"
                ...
                <div key={message.id} className="space-y-2">
                  <div className={`flex ${message.isBot ? 'justify-start' : 'justify-end'}`}>
                    <div className={`flex items-start space-x-2 max-w-[80%] ${message.isBot ? 'flex-row' : 'flex-row-reverse space-x-reverse'}`}>
                      {message.isBot ? (
                        <HannahAvatar size="md" className="flex-shrink-0" />
                      ) : (
